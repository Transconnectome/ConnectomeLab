...
result = []
gps = all_data.iloc[:, 53:79].columns
for target in gps:
    y = all_data[target]
    y.values.ravel()
    print("\n")
    ...
    #     print('='*50)
        ...
        result.append([target, loop, 0.1, 0.1, train_sc, test_sc])
        print("{:.<45}".format("1. 0.1 Train r2 score"))
        print("\t{}".format(train_sc))
        print("{:.<45}".format("2. 0.1 Test r2 score"))
...
pd.DataFrame(result, columns=['GPS', 'loop', 'C', 'gamma', 'train_score', 'test_score']).to_csv('SVM_prediction_0to1.csv', index=False)